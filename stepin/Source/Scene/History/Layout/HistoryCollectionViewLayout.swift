...
protocol HistoryCollectionViewLayoutDelegate: AnyObject {
    func collectionView(_ collectionView: UICollectionView, heightForCellIndexPath indexPath: IndexPath) -> CGFloat
}

class HistoryCollectionViewLayout: UICollectionViewFlowLayout {

    // delegate로 ViewController 를 나타낸다.
    weak var delegate: HistoryCollectionViewLayoutDelegate?
    ...
    override func prepare() {
        ...
        let xOffSet: [CGFloat] = [0, cellWidth, cellWidth * 2] // cell 의 x 위치를 나타내는 배열
        ...
        if collectionView.numberOfSections > 0 {
            for item in 0 ..< collectionView.numberOfItems(inSection: 0) {
                ...
                let imageHeight = delegate?.collectionView(collectionView, heightForCellIndexPath: indexPath) ?? 150
